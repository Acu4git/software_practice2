...
int main(int argc, char *argv[]) {
    ...
    if (strcmp(argv[1], "gpa") == 0)
        ...
        fprintf(stderr, "\033[33mError: invalid first argument \'%s\'\n",
                argv[1]);
        fprintf(stderr,
                "Please choice \'gpa\', \'credit\' or \'name\'\033[39m\n");
    }

    printf(
        "Data size: %d\n"
        "Sort time: %f sec\n",
        N, (double)(clk_end - clk_start) / CLOCKS_PER_SEC);
...
int input(char *sfname, SRec **p) {
    ...
    if ((sp = fopen(sfname, "r")) == NULL) {
        fprintf(stderr,
                "\033[33mError: failed to open the file \'%s\'\033[39m\n",
                sfname);
        exit(EXIT_FAILURE);
    }
    ...
    if ((*p = (SRec *)malloc(sizeof(SRec) * n)) == NULL) {
        ...
        fprintf(stderr,
                "Please check if the data size is appropriate.\n\033[39m");
        exit(EXIT_FAILURE);
    }

    for (int i = 0; i < n; i++) {
        int ret;
        ret = fscanf(sp, "%f %d %s", &((*p)[i].gpa), &((*p)[i].credit),
                     (*p)[i].name);
        if (ret < 3) {
            fprintf(stderr, "\033[33mError: failed to scan the records\n");
            fprintf(stderr,
                    "Please check the data format and the num of "
                    "elements.\033[39m\n");
            exit(EXIT_FAILURE);
        }
    }
